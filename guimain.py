...
class WindowMain(QWidget):
    ...
    def ExecuteLoadFile(self):        
        ...
        print("File name inside load file:", self.selected_file.split('/')[-1])
        previous_registers = getPeopleContactByFile(self.dbase, self.selected_file)
        print("Loading files, last row previous: ", len(previous_registers))
        if len(previous_registers)!=0:
            print("Is there a previous registration related to this file. Do you want to continue from the same point?")
...
class WindowsSelectCheckPoint(QWidget):
    ...
    def setContinue(self):        
        print("Continue in last check point")
        time.sleep(0.2)
        self.close()        
